...
void App::update() {
	...
	m_renderer.begin();
	{
		...
		m_shader.bind();
		m_shader.setUniform1f("uFocalLength", m_focalLength);
		m_shader.setUniform1f("uAspectRatio", RenderState::Size().aspectRatio());
		//m_shader.setUniformMat4f("uCamTransform", m_camera.transformMatrix());
		m_shader.setUniform3f("camPos", m_camera.position());
		m_shader.setUniform3f("camX", m_camera.xAxis());
		m_shader.setUniform3f("camY", m_camera.yAxis());
		m_shader.setUniform3f("camZ", m_camera.zAxis());
...
void App::ImGuiWindows() {
	...
	if (m_bShow_Debug) {
		...
		glm::mat4 m = glm::transpose(m_camera.transformMatrix());
	...
	if (!RenderState::IsExporting()) {
		...
		case SDL_KEYDOWN:
			if (!ImGui::GetIO().WantTextInput) {
				m_camera.onKeyPressed(e.key.keysym.scancode);
			}
			break;
