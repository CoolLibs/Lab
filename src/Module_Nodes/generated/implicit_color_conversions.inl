/* -----------------------------------------------------------------------------
 * This file was automatically generated by a Python script.
 * PLEASE DON'T EDIT IT DIRECTLY, your changes would be overwritten the next time the script is run.
 * Instead, go to "generator.py" and edit the "implicit_color_conversions" function there.
 * -----------------------------------------------------------------------------
 */

if (from == PrimitiveType::sRGB && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_sRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_sRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_StraightA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_PremultipliedA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_to_LinearRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_LinearRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_LinearRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_StraightA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_PremultipliedA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_sRGB_to_LinearRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_to_CIELAB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_CIELAB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_CIELAB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_StraightA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_PremultipliedA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_sRGB_to_CIELAB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_to_HSLuv/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_HSLuv_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_to_HSLuv_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_StraightA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_StraightA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_StraightA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_StraightA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_sRGB_PremultipliedA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::sRGB_PremultipliedA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_sRGB_PremultipliedA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_sRGB_PremultipliedA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_sRGB_to_HSLuv(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_to_sRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_sRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_sRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_StraightA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_PremultipliedA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_LinearRGB_to_sRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_LinearRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_LinearRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_StraightA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_PremultipliedA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_to_CIELAB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_CIELAB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_CIELAB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_StraightA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_PremultipliedA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_LinearRGB_to_CIELAB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_to_HSLuv/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_HSLuv_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_to_HSLuv_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_StraightA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_StraightA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_StraightA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_StraightA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_LinearRGB_PremultipliedA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::LinearRGB_PremultipliedA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_LinearRGB_PremultipliedA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_LinearRGB_PremultipliedA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_LinearRGB_to_HSLuv(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_to_sRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_sRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_sRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_StraightA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_PremultipliedA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_CIELAB_to_sRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_to_LinearRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_LinearRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_LinearRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_StraightA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_PremultipliedA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_CIELAB_to_LinearRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_CIELAB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_CIELAB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_StraightA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_PremultipliedA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_to_HSLuv/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_HSLuv_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_to_HSLuv_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_StraightA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_StraightA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_StraightA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_StraightA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_CIELAB_PremultipliedA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::CIELAB_PremultipliedA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_CIELAB_PremultipliedA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_CIELAB_PremultipliedA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_CIELAB_to_HSLuv(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_to_sRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_sRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_sRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_StraightA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::sRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_sRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_PremultipliedA_to_sRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::sRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_sRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_sRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::sRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_sRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_sRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_HSLuv_to_sRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_to_LinearRGB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_LinearRGB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_LinearRGB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_StraightA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::LinearRGB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_LinearRGB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_PremultipliedA_to_LinearRGB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::LinearRGB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_LinearRGB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_LinearRGB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::LinearRGB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_LinearRGB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_LinearRGB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_HSLuv_to_LinearRGB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_to_CIELAB/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_CIELAB_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_CIELAB_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_StraightA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::CIELAB)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_CIELAB",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_PremultipliedA_to_CIELAB/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::CIELAB_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_CIELAB_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_CIELAB_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::CIELAB_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_CIELAB_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_CIELAB_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        // We need to unpremultiply for the color conversion, and re-premultiply afterwards
                        vec3 to = Cool_HSLuv_to_CIELAB(from.xyz / from.a);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_HSLuv_StraightA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, coollab_context.alpha);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_to_HSLuv_PremultipliedA/*coollabdef*/(vec3 from)
                            {
                                
                        vec3 to = (from);
                        return vec4(to, 1.) * coollab_context.alpha;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_StraightA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_StraightA && to == PrimitiveType::HSLuv_PremultipliedA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_StraightA_to_HSLuv_PremultipliedA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_StraightA_to_HSLuv_PremultipliedA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return vec4(to * from.a, from.a);
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::HSLuv)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_HSLuv",
        .implementation = R"STR(
                            vec3 CoolLab_HSLuv_PremultipliedA_to_HSLuv/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz);
                        return to;
                    
                            }
                            )STR",
    });
}

if (from == PrimitiveType::HSLuv_PremultipliedA && to == PrimitiveType::HSLuv_StraightA)
{
    return context.push_function({
        .name           = "CoolLab_HSLuv_PremultipliedA_to_HSLuv_StraightA",
        .implementation = R"STR(
                            vec4 CoolLab_HSLuv_PremultipliedA_to_HSLuv_StraightA/*coollabdef*/(vec4 from)
                            {
                                
                        vec3 to = (from.xyz / from.a);
                        return vec4(to, from.a);
                    
                            }
                            )STR",
    });
}
