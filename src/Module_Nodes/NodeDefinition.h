...
#include "ValueInput_Definition.h"
...
struct NodeDefinition_Data {
    ...
    std::vector<FunctionInput_Definition> function_inputs{}; // Things that can only come from a pin
    std::vector<ValueInput_Definition>    value_inputs{};    // Things that will default to a widget on the node if nothing is plugged into the pin
...
public:
    ...
    [[nodiscard]] auto function_inputs() const -> auto const& { return _data.function_inputs; }
    [[nodiscard]] auto value_inputs() const -> auto const& { return _data.value_inputs; }
