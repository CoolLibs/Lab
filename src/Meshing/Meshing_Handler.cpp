...
void Meshing_Handler::imgui_windows(Ui_Ref const& /* ui */, UpdateContext_Ref const& /*update_ctx */) const
{
    ImGui::Begin("Meshing");
...
void Meshing_Handler::generate_mesh_if_needed(
    ...
    if (_needs_to_compute_mesh) {
        compute_mesh(feedback_double_buffer, camera_input, nodes_graph, get_node_definition, input_provider, node_id);
...
void Meshing_Handler::compute_mesh(
    ...
    auto shader_code { generate_meshing_shader_code(nodes_graph, node_id, get_node_definition, input_provider) };
    ...
    set_uniforms_for_shader_based_module(meshing_compute_shader.value(), input_provider, {}, feedback_double_buffer, camera_input, nodes_graph);
