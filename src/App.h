...
private:
    ...
    auto update_context                             () { return UpdateContext_Ref{{set_clean__functor(), set_dirty__functor(), input_provider(0.f, 0.f, -100000.f, 0.f, _project.camera2D.value().transform_matrix(1.f) /* HACK: Dummy values, they should not be needed. Currently this is only used by shader code generation to inject of very specific types like Gradient */), ui(), _nodes_library_manager.library()}}; }
    // clang-format on
